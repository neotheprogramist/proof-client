...
| `serve` / `attest` | Verifier identity / HTTPS request and disclosure policy             | Redacted HTTP transcripts / full private HTTP transcripts |
...
Shell examples use zsh or bash. Run every terminal from the repository root. CLI examples use `cargo run --release --locked --bin proof-client -- ...`. No browser build step, browser tests, custom lint-rule suite or CI. Examples create files under ignored `target/manual/`. Each run requires a fresh output directory; existing artifacts are never overwritten. Proof examples use `2 × available CPUs + 1` worker threads, derived with Node.js.
...
cargo run --release --locked --bin proof-client -- prepare --circuit examples/merkle/base.json --output target/manual/base/metadata.json --threads "$threads"
cargo run --release --locked --bin proof-client -- prove --circuit examples/merkle/base.json --public target/manual/base/public.json --witness target/manual/base/witness.json --output target/manual/base/proof.json --threads "$threads"
cargo run --release --locked --bin proof-client -- verify --circuit examples/merkle/base.json --public target/manual/base/public.json --proof target/manual/base/proof.json --threads "$threads"
)
```
...
cargo run --release --locked --bin proof-client -- prepare --circuit examples/merkle/merge-recursive.json --output target/manual/tree/metadata.json --threads "$threads"
for i in 0 1 2 3 4 5 6 7; do
  ...
  cargo run --release --locked --bin proof-client -- prove --circuit examples/merkle/base.json --public "target/manual/tree/0-$i.public.json" --witness "target/manual/tree/0-$i.witness.json" --output "target/manual/tree/0-$i.proof.json" --threads "$threads"
done
for height in 1 2 3; do
  ...
  while [ "$i" -lt "$count" ]; do
    ...
    cargo run --release --locked --bin proof-client -- prove --circuit "examples/merkle/$circuit.json" --public "target/manual/tree/$height-$i.public.json" --witness "target/manual/tree/$height-$i.witness.json" --output "target/manual/tree/$height-$i.proof.json" --threads "$threads"
    i=$((i + 1))
  done
done
...
cargo run --release --locked --bin proof-client -- verify --circuit examples/merkle/merge-recursive.json --public target/manual/tree/expected.json --proof target/manual/tree/3-0.proof.json --threads "$threads"
)
```
...
`serve` saves redacted sent/received transcripts before acknowledging them. `attest` checks the protocol receipt, then saves the **full private HTTP request and response** locally. Both commands require distinct `--sent-output` and `--recv-output` paths. These `.txt` files preserve original HTTP bytes, including CRLF and body formatting. The core renderer defines `HIDDEN_BYTE` (🙈) and `COMMITTED_BYTE` (🔒), one marker per hidden or committed byte, including hidden line breaks. Binary bytes remain binary. Redacted files are readable views, not replayable HTTP messages or portable proofs; emoji change byte offsets. `--metadata-output` saves pretty-printed JSON with the session, server, original lengths, and authenticated commitment digests/ranges. `attest` also requires `--secrets-output` for private opening secrets. These secrets and the original private transcripts allow later opening of the commitments; they are never sent to the verifier. Limits, marker constants and commitment policy live in `crates/core/src/tls/attest.rs`; QUIC deadlines live in `crates/core/src/tls/quic.rs`.

Each file is published without overwriting; publishing the output set and delivering acknowledgement are not atomic. A failure can leave a subset of outputs, or all verifier files without client acknowledgement. Requests may affect the target before failure. Nothing retries automatically. On failure/cancellation, inspect output paths; forced termination during publication can leave a private temporary file. Published files survive cancellation. Filesystem access is outside the privacy boundary.
...
```sh
cargo run --release --locked --bin proof-client -- serve --listen 127.0.0.1:7047 --cert target/manual/fixture/verifier.pem --key target/manual/fixture/verifier.key --target-ca target/manual/fixture/target.pem --server-name localhost --session fixture --sent-output target/manual/fixture/verified-sent.txt --recv-output target/manual/fixture/verified-recv.txt --metadata-output target/manual/fixture/verified-metadata.json
```

Wait for the Ready event, then in terminal 3:

```sh
cargo run --release --locked --bin proof-client -- attest --request target/manual/fixture/request.json --disclosure target/manual/fixture/disclosure.json --verifier 127.0.0.1:7047 --verifier-name localhost --verifier-ca target/manual/fixture/verifier.pem --target-ca target/manual/fixture/target.pem --session fixture --sent-output target/manual/fixture/private-sent.txt --recv-output target/manual/fixture/private-recv.txt --metadata-output target/manual/fixture/private-metadata.json --secrets-output target/manual/fixture/private-secrets.json
```

Expect disclosed HTTP lines, `"AvailableBalance":42.1200` and `"currency":"PLN"`, 🙈 for hidden bytes (including cookies), and 🔒 for the committed account field. Neither cookies nor `PRIVATE` appear in verifier outputs. The private response includes the hidden account field, and the private request includes its cookies. All three processes exit after one completed attestation. Mismatched identity/session/trust or an occupied destination fails.
...
Each port sends one invocation, `{"protocol":"proof-client/8","args":[...]}`, using CLI arguments. Frames are UTF-8 JSON with a native-endian four-byte length, bounded by `MAX_FRAME_BYTES` in `crates/cli/src/stdio.rs`. Serve emits Ready, then every invocation emits Completed or Failed and exits. Terminal events use JSON lines; terminal errors use stderr and failure status. Arguments never pass through a shell.
