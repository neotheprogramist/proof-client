...
pub(crate) fn poseidon1_artifact_air<SC>(
...
where
    ...
    SymbolicExpressionExt<Val<SC>, SC::Challenge>:
        Algebra<SymbolicExpression<Val<SC>>> + Algebra<SC::Challenge>,
{
    Poseidon1Prover::new(config, Default::default()).wrapper_from_config_with_preprocessed(
        Vec::new(),
        min_height,
...
impl Poseidon1Prover {
    ...
    where
        ...
        SymbolicExpressionExt<Val<SC>, SC::Challenge>:
            Algebra<SymbolicExpression<Val<SC>>> + Algebra<SC::Challenge>,
    {
        let op_type = NpoTypeId::poseidon1_perm(self.config);
        let t = traces.non_primitive_trace::<Poseidon1Trace<Val<SC>>>(&op_type)?;
